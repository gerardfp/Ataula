...
function loadCellEditingTestCases(): CellEditingTestCase[] {
  ...
  const cases: CellEditingTestCase[] = [];
  let currentCase: Partial<CellEditingTestCase> = {};
  let currentSection: 'before' | 'content' | 'after' | null = null;
  let currentLines: string[] = [];
...
describe('Table Cell Content Editing Integration Tests', () => {
  ...
  testCases.forEach((tc, idx) => {
    it(`should simulate Content Editing Case ${idx + 1} correctly`, () => {
      ...
      let tableNode = parseGeometricTable(tableStr, false, true);
      ...
      tableNode = simplifyTable(tableNode);
      const formatted = formatGeometricTable(tableNode, true);
      const expected = tc.after.join('\n');
