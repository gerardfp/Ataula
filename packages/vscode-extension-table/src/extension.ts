...
export function activate(context: vscode.ExtensionContext) {
  ...
  async function runLiveFormatting(currentEditor: vscode.TextEditor, document: vscode.TextDocument) {
    ...
    if (activeTable && activeTable.documentUri.fsPath === document.uri.fsPath && activeTable.targetCursor) {
      ...
      let formattedTable;
      try {
        formattedTable = formatGeometricTable(activeTable.tableNode);
      } catch (e: any) {
  ...
  vscode.workspace.onDidChangeTextDocument(async event => {
    ...
    if (!activeTable) {
      ...
      if (currentLineIdx < event.document.lineCount && event.document.lineAt(currentLineIdx).text.trim().startsWith('|')) {
        ...
        const tableStr = tableLines.join('\n');
        try {
          const tableNode = parseGeometricTable(tableStr, false, true);
          if (tableNode && tableNode.cells.length > 0) {
            const maxLength = Math.max(...tableLines.map(line => line.length));
            const grid = tableLines.map(line => line.padEnd(maxLength, ' '));
            const hLines: number[] = [];
...
function getMinLeadingSpacesForCell(
  cell: TableCell,
  startLineIdx: number,
  hLines: number[],
  vLines: number[],
...
function getSingleCellForChange(
  tableNode: TableNode,
  ...
  const cell = tableNode.cells.find(
    (c: any) => c.row <= startJ && startJ < c.row + c.rowspan && c.column <= startI && startI < c.column + c.colspan
  );
  ...
  const endCell = tableNode.cells.find(
    (c: any) => c.row <= endJ && endJ < c.row + c.rowspan && c.column <= endI && endI < c.column + c.colspan
  );
