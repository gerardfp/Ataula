...
function activate(context) {
  ...
  console.log("La extensi\xF3n EnTaula est\xE1 activa.");
  ...
  async function runLayoutFormatting(currentEditor, document) {
    ...
    if (expandedAny) {
      ...
      try {
        ...
        success2 = await applyWorkspaceEdit(workspaceEdit);
      } catch (err) {
  ...
  vscode.workspace.onDidChangeTextDocument(async (event) => {
    ...
    if (isFormatting) {
      pendingFormat = true;
      for (const change2 of event.contentChanges) {
        bufferedChanges.push(change2);
      }
      return;
    }
  ...
  const convertToEnTaulaCommand = vscode.commands.registerCommand("entaula.convertToEnTaula", async () => {
    ...
    try {
      formattedEnTaulaTable = (0, import_table_engine.formatGeometricTable)(tableNode);
    } catch (e) {
