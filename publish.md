...
2. **Crear un perfil de Editor (Publisher)**:
   ...
   - Crea un **Publisher ID** con el nombre exacto: **`gfalcoperez`** (debe coincidir con la propiedad `"publisher"` de tu [`package.json`](./packages/vscode-extension-table/package.json)).
...
2. Haz clic sobre tu perfil **`gfalcoperez`**.
3. Haz clic en **New Extension** -> **Visual Studio Code**.
4. Arrastra y suelta el archivo `.vsix` recién generado (`packages/vscode-extension-table/entaula-1.0.0.vsix`).
...
1. Desde la carpeta de la extensión (`packages/vscode-extension-table`), inicia sesión introduciendo tu token PAT:
   ```bash
   npx @vscode/vsce login gfalcoperez
   ```
